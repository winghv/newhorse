...
- `pass`: 总分 `>= 37`，且没有任何单项低于 `3`
- `revise`: 总分在 `29-36` 之间，或存在任一单项等于 `2`
- `block`: 总分 `<= 28`，或 `hook_strength` / `opening_hold_power` / `proof_strength` / `platform_fit` 任一项 `<= 1`

额外规则：

- 即使总分达标，只要 `hook_strength`、`opening_hold_power`、`proof_strength`、`platform_fit` 任一项低于 `3`，也不能进入发布
